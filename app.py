...
class Planetoids(GameApp):
    ...
    def start(self):
        ...
        if self._state == STATE_INACTIVE:
            self._title = GLabel(text = 'Planetoids', font_size=TITLE_SIZE, font_name=TITLE_FONT, x=GAME_WIDTH/2, y= GAME_HEIGHT/2)
            self._message = GLabel(text = "Press 'x' to continue", font_size=MESSAGE_SIZE, font_name=MESSAGE_FONT, x=GAME_WIDTH/2, y= GAME_HEIGHT/3)
            self._gameOver = GLabel(text = 'GAME OVER', font_size=TITLE_SIZE, font_name=TITLE_FONT, x=GAME_WIDTH/2, y= GAME_HEIGHT/2)
            self._nextTime = GLabel(text = 'Better luck next time!', font_size=MESSAGE_SIZE, font_name=MESSAGE_FONT, x=GAME_WIDTH/2, y= GAME_HEIGHT/3)
    ...
    def update(self,dt):
        ...
        """
        if self._state == STATE_INACTIVE and self.input.is_key_pressed('x'):
            ...
            if self._wave.checkWin() != None: # game over
                self._state = STATE_COMPLETE
        elif self._state == STATE_PAUSED:
            self._gameOver = GLabel(text = 'GAME OVER', font_size=TITLE_SIZE, font_name=TITLE_FONT, x=GAME_WIDTH/2, y= GAME_HEIGHT/2)
            self._nextTime = GLabel(text = "Press 'x' to play again", font_size=MESSAGE_SIZE, font_name=MESSAGE_FONT, x=GAME_WIDTH/2, y= GAME_HEIGHT/3)
            self._state == STATE_INACTIVE
        elif self._state == STATE_COMPLETE:
            if self._wave.checkWin()  == 'win':
                self._message = GLabel(text="YOU WIN",font_size = TITLE_SIZE/2, font_name = MESSAGE_FONT, x=400, y=300)
            else:
                self._message = GLabel(text="YOU LOSE",font_size = TITLE_SIZE/2, font_name = MESSAGE_FONT, x=400, y=300)
    ...
    def draw(self):
        ...
        if self._state == STATE_INACTIVE:
            ...
            self.prepareView()
            self._wave.draw(self.view)
            if len(self._wave.getAsteroids()) <= 0:
                self._message = GLabel(text="YOU WIN",font_size = TITLE_SIZE/2, font_name = MESSAGE_FONT, x=400, y=300)
            else:
                self._message = GLabel(text="YOU LOSE",font_size = TITLE_SIZE/2, font_name = MESSAGE_FONT, x=400, y=300)
            self._message.draw(self.view)
